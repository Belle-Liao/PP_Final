...
template<int n> struct vec {
    double data[n] = {0};
    CUDA_HOSTDEV double& operator[](const int i)       { assert(i>=0 && i<n); return data[i]; }
    CUDA_HOSTDEV double  operator[](const int i) const { assert(i>=0 && i<n); return data[i]; }
};
...
template<> struct vec<2> {
    double x = 0, y = 0;
    CUDA_HOSTDEV double& operator[](const int i)       { assert(i>=0 && i<2); return i ? y : x; }
    CUDA_HOSTDEV double  operator[](const int i) const { assert(i>=0 && i<2); return i ? y : x; }
};

template<> struct vec<3> {
    double x = 0, y = 0, z = 0;
    CUDA_HOSTDEV double& operator[](const int i)       { assert(i>=0 && i<3); return i ? (1==i ? y : z) : x; }
    CUDA_HOSTDEV double  operator[](const int i) const { assert(i>=0 && i<3); return i ? (1==i ? y : z) : x; }
};

template<> struct vec<4> {
    double x = 0, y = 0, z = 0, w = 0;
    CUDA_HOSTDEV double& operator[](const int i)       { assert(i>=0 && i<4); return i<2 ? (i ? y : x) : (2==i ? z : w); }
    CUDA_HOSTDEV double  operator[](const int i) const { assert(i>=0 && i<4); return i<2 ? (i ? y : x) : (2==i ? z : w); }
    CUDA_HOSTDEV vec<2> xy()  const { return {x, y};    }
    CUDA_HOSTDEV vec<3> xyz() const { return {x, y, z}; }
};
...
template<int nrows,int ncols> struct mat {
    ...
    CUDA_HOSTDEV       vec<ncols>& operator[] (const int idx)       { assert(idx>=0 && idx<nrows); return rows[idx]; }
    CUDA_HOSTDEV const vec<ncols>& operator[] (const int idx) const { assert(idx>=0 && idx<nrows); return rows[idx]; }
